...
void setgpio(char *pin, char *etat)
{
    ...
    write(fd, pin, strlen(pin));    // Écrit le numéro du gpio voulu dans ce fichier pour l'exporté
    ...
    strcat(fichierValeur,pin);
    strcat(fichierValeur,valeur);

    if(strcmp(etat,"on") == 0)           // Si en paramètre on a écrit "on"
    {
    ...
    else if (strcmp(etat,"off") == 0)    // Si en paramètre on a écrit "off"
    {
        fd = open(fichierValeur, O_WRONLY); // Ouvre le fichier pour changer l'état du gpio voulu
    ...
    write(fd,pin,strlen(pin));      // Écrit le numéro du gpio voulu pour l'unexport 
    fd = close(fd);                         // Ferme ce fichier
